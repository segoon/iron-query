...
public:
  ...
  /// (optionally dotted) SQL identifier not already used by an earlier CTE
  /// in this WITH clause.
  /// @throws InvalidIdentifier if `name` is not a valid identifier.
  /// @throws LogicError if `name` was already used by an earlier CTE.
...
private:
  ...
  std::string ctes_;
  std::string ctes_formatted_;
  std::vector<std::string> cte_names_;
